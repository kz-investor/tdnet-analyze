...
# 市場フィルタ用
try:
    from tdnet_analyzer.common.constants import load_company_market_map, EXCLUDED_MARKETS_DEFAULT, normalize_code
except Exception:
...
def scrape(request):
    ...
    try:
        ...
        scraper = TDNetCloud(config_path=str(project_path('config', 'config.yaml')), use_multithread=True)
...
class TDNetCloud(TDNetBase):
    ...
    def __init__(self, config_path: str | None = None, use_multithread: bool = True):
        ...
        if load_company_market_map and companies_csv_path.exists():
            try:
